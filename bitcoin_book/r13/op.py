...
def op_if(stack, items):
    if len(stack) < 1:
        return False
    # jeszcze raz i odtwórz tablicę elementów w oparciu o elementu ze szczytu stosu
    true_items = []
    false_items = []
    ...
    while len(items) > 0:
        item = items.pop(0)
        if item in (99, 100):
            # zagnieżdżona instrukcja if, musimy przejść do innego endif
            num_endifs_needed += 1
...
def op_checkmultisig(stack, z):
    ...
    der_signatures = []
    for _ in range(m):
        # zakłada się, że podpis używa SIGHASH_ALL
        der_signatures.append(stack.pop()[:-1])
    # błąd OP_CHECKMULTISIG 
    stack.pop()
    try:
        # przetwórz wszystkie punkty
        points = [S256Point.parse(sec) for sec in sec_pubkeys]
        # przetwórz wszystkie podpisy
        sigs = [Signature.parse(der) for der in der_signatures]
        # wykonaj pętlę dla wszystkich podpisów
        for sig in sigs:
            # jeśli zabrakło punktów, podpisy nie przydadzą się
            if len(points) == 0:
                LOGGER.info("podpisy niepoprawne lub w błędnej kolejności")
                return False
            # wykonaj pętlę, aż znajdziemy punkt, który działa z tym podpisem
            while points:
                # weź bieżący punkt z listy punktów
                point = points.pop(0)
                # sprawdź, czy ten podpis pasuje do bieżącego punktu
                if point.verify(z, sig):
                    break
        # podpisy są poprawne, więc odłóż 1 na stos
        stack.append(encode_num(1))
