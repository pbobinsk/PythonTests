...
class TxFetcher:
    ...
    def fetch(cls, tx_id, testnet=False, fresh=False):
        if fresh or (tx_id not in cls.cache):
            ...
            if computed != tx_id:
                raise RuntimeError('błędna odpowiedź serwera: {} vs {}'.format(computed, tx_id))
            cls.cache[tx_id] = tx
...
class Tx:
    ...
    # tag::source5[]
    def hash(self):
        '''Binarny skrót ze starej serializacji'''
    ...
    def parse_segwit(cls, s, testnet=False):
        ...
        if marker != b'\x00\x01':  # <1>
            raise RuntimeError('To nie jest transakcja Segwita {}'.format(marker))
        num_inputs = read_varint(s)
        inputs = []
        for _ in range(num_inputs):
            inputs.append(TxIn.parse(s))
    ...
    def sig_hash(self, input_index, redeem_script=None):
        ...
        for i, tx_in in enumerate(self.tx_ins):
            ...
            if i == input_index:
                # jeśli przekazany został RedeemScript, jest ScriptSigiem;
                if redeem_script:
                    script_sig = redeem_script
                # w przeciwnym wypadku ScriptSigiem jest ScriptPubkey poprzedniej tx
                else:
    ...
    def hash_sequence(self):
        if self._hash_sequence is None:
            self.hash_prevouts()  # powinno to obliczać self._hash_prevouts
    ...
    def sig_hash_bip143(self, input_index, redeem_script=None, witness_script=None):
        '''Zwraca, jako liczbę całkowitą, skrót, który musi zostać podpisany dla indeksu input_index'''
        tx_in = self.tx_ins[input_index]
        # według specyfikacji BIP143
        s = int_to_little_endian(self.version, 4)
    ...
    def verify_input(self, input_index):
        ...
        # sprawdź, czy ScriptPubkey to p2sh
        if script_pubkey.is_p2sh_script_pubkey():
            # ostatnie polecenie w p2sh to RedeemScript
            cmd = tx_in.script_sig.cmds[-1]
            # przetwórz RedeemScript
            raw_redeem = int_to_little_endian(len(cmd), 1) + cmd
            redeem_script = Script.parse(BytesIO(raw_redeem))
            # RedeemScript może być p2wpkh lub p2wsh
            ...
            # ScriptPubkey może być p2wpkh lub p2wsh
    ...
    def verify(self):
        ...
        # sprawdź, czy nie tworzymy pieniędzy
        if self.fee() < 0:
            return False
        # sprawdź, czy każde wejście posiada prawidłowy ScriptSig
    ...
    def sign_input(self, input_index, private_key):
        '''Podpisuje wejście za pomocą klucza prywatnego'''
        # weź skrót podpisu (z)
        z = self.sig_hash(input_index)
    ...
    def is_coinbase(self):
        '''Sprawdza, czy transakcja jest transakcją coinbase, czy nie jest'''
        # sprawdź, czy jest dokładnie jedno wejście
        if len(self.tx_ins) != 1:
            return False
        # weź pierwsze wejście
        first_input = self.tx_ins[0]
        # sprawdź, czy prev_tx pierwszego wejścia wynosi b'\x00' * 32 bytes
        if first_input.prev_tx != b'\x00' * 32:
            return False
        # sprawdź, czy  prev_index pierwszego wejścia wynosi 0xffffffff
        if first_input.prev_index != 0xffffffff:
            return False
        return True

    def coinbase_height(self):
        '''Zwraca wysokość bloku, w którym znajduje się ta transakcja coinbase
        Zwraca None, jeśli transakcja ta nie jest transakcją coinbase'''
        # jeśli NIE jest to transakcja coinbase, zwróć None
        if not self.is_coinbase():
            return None
        # weź pierwsze polecenie
        first_cmd = self.tx_ins[0].script_sig.cmds[0]
        # skonwertuj je z wartości w porządku little endian na int
