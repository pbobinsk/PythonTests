...
class Point:
    ...
    def __init__(self, x, y, a, b):
        ...
        self.x = x
        self.y = y
        # x równe None i y równe None - oznacza punkt w nieskończoności
        # Sprawdź to tutaj, ponieważ poniższe równanie nie będzie miało sensu
        # z wartościami None dla obu.
        if self.x is None and self.y is None:
            return
        # sprawdź, czy spełnione jest równanie krzywej eliptycznej
        # y**2 == x**3 + a*x + b
        if self.y**2 != self.x**3 + a * x + b:
            # jeśli nie, wygeneruj błąd ValueError
...
class ECCTest(TestCase):
    ...
    def test_on_curve(self):
        # sprawdza następujące punkty, czy leżą 
        # na krzywej y^2=x^3-7 w ciele skończonym F_223:
        # (192,105) (17,56) (200,119) (1,193) (42,99)
        # te, które nie leżą, powinny wygenerować błąd ValueError
        prime = 223
        ...
        # iteruj po poprawnych punktach
        for x_raw, y_raw in valid_points:
            x = FieldElement(x_raw, prime)
            y = FieldElement(y_raw, prime)
            # Tworzenie punktu nie powinno kończyć się błędem
            Point(x, y, a, b)

        # iteruj po niepoprawnych punktach
    ...
    def test_add(self):
        # testuje dodawanie na krzywej y^2=x^3-7 w ciele F_223:
        # (192,105) + (17,56)
        # (47,71) + (117,141)
        # (143,98) + (76,66)
        prime = 223
    ...
    def test_rmul(self):
        ...
        # iteruj po mnożeniach
        for s, x1_raw, y1_raw, x2_raw, y2_raw in multiplications:
            x1 = FieldElement(x1_raw, prime)
            y1 = FieldElement(y1_raw, prime)
            p1 = Point(x1, y1, a, b)
            # zainicjuj drugi punkt zależnie od tego, czy jest to punkt w nieskończoności
            if x2_raw is None:
            ...
            # sprawdź, czy iloczyn jest równy spodziewanemu punktowi
...
class S256Point(Point):
    ...
    def verify(self, z, sig):
        # Według małego twierdzenia Fermata 1/s = pow(s, N-2, N)
        s_inv = pow(sig.s, N - 2, N)
        # u = z / s
        u = z * s_inv % N
        # v = r / s
        v = sig.r * s_inv % N
        # współrzędną x równania u*G + v*P powinna być wartość r
        total = u * G + v * self
        return total.x.num == sig.r

    def sec(self, compressed=True):
        '''Zwraca postać binarną formatu SEC'''
        # jeśli jest skompresowany, zaczyna się od b'\x02', gdy self.y.num jest parzysta, lub od b'\ x03', gdy self.y jest nieparzysta
        # wtedy self.x.num
        # pamiętaj, że musisz przekonwertować self.x.num/self.y.num na wartości binarne (jakaś_liczba_całkowita.to_bytes (32, 'big'))
        if compressed:
            if self.y.num % 2 == 0:
            ...
            # jeśli jest nieskompresowany, zaczyna się od b'\ x04’ i self.x, a następnie self.y
...
class S256Test(TestCase):
    ...
    def test_pubpoint(self):
        # napisz test, który sprawdza punkt publiczny dla następujących wartości
        points = (
            # secret, x, y
        ...
        for secret, x, y in points:
            # zainicjuj punkt secp256k1 (S256Point)
            point = S256Point(x, y)
            # sprawdź, czy tajna liczba * G jest tą samą wartością, co punkt
...
class PrivateKey:
    ...
    def sign(self, z):
        ...
        # r jest współrzędną x wynikowego punktu k*G
        r = (k * G).x.num
        # pamiętaj: 1/k = pow(k, N-2, N)
        k_inv = pow(k, N - 2, N)
        # s = (z+r*secret) / k
        s = (z + r * self.secret) * k_inv % N
        if s > N / 2:
            s = N - s
        # zwróć instancję Signature Signature:
        # Signature(r, s)
    ...
    def wif(self, compressed=True, testnet=False):
        # przekonwertuj tajną liczbę z liczby całkowitej na postać 32-bajtową w porządku big endian, używając num.to_bytes (32, 'big')
        secret_bytes = self.secret.to_bytes(32, 'big')
        # wstaw na początek b'\xef' dla testnetu lub b '\ x80' dla mainnetu
        if testnet:
            prefix = b'\xef'
        else:
            prefix = b'\x80'
        # wstaw na początek b'\x01', jeśli jest skompresowany
        if compressed:
            suffix = b'\x01'
        else:
            suffix = b''
        # zakoduj całość metodą encode_base58_checksum
