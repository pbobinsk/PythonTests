...
class MerkleTree:
    ...
    def __init__(self, total):
        self.total = total
        # oblicz maksymalną głębokość math.ceil(math.log (self.total, 2))
        self.max_depth = math.ceil(math.log(self.total, 2))
        # zainicjuj właściwość nodes, która ma przechowywać drzewo
        self.nodes = []
        # wykonaj w pętli dla kolejnych poziomów (max_depth+1)
        for depth in range(self.max_depth + 1):
            # liczba elementów na tej głębokości wynosi
            # math.ceil(self.total / 2**(self.max_depth - depth))
            num_items = math.ceil(self.total / 2**(self.max_depth - depth))
            # utwórz listę skrótów tego poziomu z odpowiednią liczbą elementów
            level_hashes = [None] * num_items
            # dodaj skróty tego poziomu do drzewa skrótów
            self.nodes.append(level_hashes)
        # ustaw wskaźnik na korzeń (głębokość = 0, indeks = 0)
    ...
    def up(self):
        # zmniejsz głębokość o 1 i zmniejsz indeks o połowę
        self.current_depth -= 1
        self.current_index //= 2

    def left(self):
        # zwiększ głębokość o 1 i podwój indeks
        self.current_depth += 1
        self.current_index *= 2

    def right(self):
        # zwiększ głębokość o 1 i podwój indeks + 1
        self.current_depth += 1
    ...
    def populate_tree(self, flag_bits, hashes):
        # wypełniaj, aż do osiągnięcia korzenia
        while self.root() is None:
            # jeśli to liść, znamy skrót tej pozycji
            if self.is_leaf():
                # pobierz następny bit za pomocą flag_bits: flag_bits.pop (0)
                flag_bits.pop(0)
                # ustaw bieżący węzeł w drzewie skrótów na następny skrót: hashes.pop (0)
                ...
                # jeśli nie mamy lewego skrótu
                if left_hash is None:
                    # jeśli następny bit flagi ma wartość 0, to następny skrót jest naszym bieżącym węzłem
                    if flag_bits.pop(0) == 0:
                        # ustaw bieżący węzeł, jako następny skrót
                        self.set_current_node(hashes.pop(0))
                        # to pod-drzewo nie wymaga obliczeń, idź wyżej
                        self.up()
                    else:
                        # przejdź do lewego węzła
                        self.left()
                elif self.right_exists():
                    # pobierz prawy skrót
                    right_hash = self.get_right_node()
                    # jeśli nie mamy prawego skrótu
                    if right_hash is None:
                        # przejdź do prawego węzła
                        self.right()
                    else:
                        # scal lewy i prawy skrót
                        self.set_current_node(merkle_parent(left_hash, right_hash))
                        # ukończyliśmy to pod-drzewo, idź wyżej
                        self.up()
                else:
                    # połącz lewy skrót dwa razy
                    self.set_current_node(merkle_parent(left_hash, left_hash))
                    # ukończyliśmy to pod-drzewo, idź wyżej
                    self.up()
        if len(hashes) != 0:
            raise RuntimeError('Nie przetworzono wszystkich skrótów {}'.format(len(hashes)))
        for flag_bit in flag_bits:
            if flag_bit != 0:
                raise RuntimeError('Nie przetworzono wszystkich bitów')
...
class MerkleBlock:
    ...
    @classmethod
    def parse(cls, s):
        '''Przetwarza blok drzewa skrótów w strumieniu bajtów. Zwraca obiekt typu MerkleBlock'''
        # wersja  - 4 bajty, little endian, int
        ...
        # znacznik czasu - 4 bajty, little endian, int
        timestamp = little_endian_to_int(s.read(4))
        # bity - 4 bajty
        bits = s.read(4)
        # nonce - 4 bajty
        nonce = s.read(4)
        # liczba transakcji w bloku - 4 bajty, little endian
        total = little_endian_to_int(s.read(4))
        # liczba skrótów transakcji - varint
        num_hashes = read_varint(s)
        # każda transakcja, to 32 bajty, little endian
        hashes = []
        for _ in range(num_hashes):
            hashes.append(s.read(32)[::-1])
        # długość pola flag - varint
        flags_length = read_varint(s)
        # przeczytaj pole flag
    ...
    def is_valid(self):
        '''Sprawdza, czy informacje o drzewie skrótów potwierdzają poprawność korzenia tego drzewa'''
        # skonwertuj pole flag na pole bitowe
        flag_bits = bytes_to_bit_field(self.flags)
        # odwróć self.hashes aby obliczyć korzeń drzewa skrótów
        hashes = [h[::-1] for h in self.hashes]
        # zainicjuj drzewo skrótów
        merkle_tree = MerkleTree(self.total)
        # wypełnij drzewo bitami flag i skrótami
        merkle_tree.populate_tree(flag_bits, hashes)
        # sprawdź, czy obliczony korzeń po odwróceniu jest taki sam jak korzeń drzewa skrótów
