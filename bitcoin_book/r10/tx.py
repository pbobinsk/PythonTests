...
class Tx:
    ...
    def verify_input(self, input_index):
        ...
        # sprawdź, czy ScriptPubkey to p2sh, używając
        # Script.is_p2sh_script_pubkey()
        if script_pubkey.is_p2sh_script_pubkey():
            # ostatnie polecenie w p2sh to RedeemScript
            cmd = tx_in.script_sig.cmds[-1]
            # dopisz na początku długość RedeemScript, używając encode_varint
            raw_redeem = encode_varint(len(cmd)) + cmd
            # przetwórz RedeemScript
            redeem_script = Script.parse(BytesIO(raw_redeem))
        # w przeciwnym wypadku RedeemScript ma wartość None
        else:
            redeem_script = None
        # weź skrót podpisu (z)
        # przekaż RedeemScript do funkcji sig_hash
